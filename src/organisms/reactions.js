...
export class _ReactionsElement extends LitElement {
  ...
  _renderIcon (data) {
    return data.icon ? (html`<img alt=${data.name} src=${data.icon} />`) : this.emoji.get(data.name)
  }
    ...
    return !shouldRender
      ...
      : (html`
        <div class='icon' name=${k} @click=${this._handleClickBounded}>
          ...
          ${shouldCount ? html`<span class='count'>${count}</span>` : undefined}
        </div>
      `)
  }
...
export const ReactionsElement = withStyle(html)(
  _ReactionsElement,
  style
)
